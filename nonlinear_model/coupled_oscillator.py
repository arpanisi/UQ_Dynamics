...
class Lorenz3D(CoupledOscillator):
    ...
    def _velocity_function(self, x0):
        ...
        for k in range(3, self.dimension + 3):
            ...
            osc_ind = int(i / 3)
            if i % 3 == 0:
                dy_dx[k - 3] = self.parameter[osc_ind, 0] * (X0[i+1] - X0[i]) + self.epsilon * (X0[i+3] - 2*X0[i] + X0[i-3])
            elif i % 3 == 1:
                dy_dx[k - 3] = X0[i - 1] * (self.parameter[osc_ind, 1] - X0[i + 1]) - X0[i]
            elif i % 3 == 2:
                dy_dx[k - 3] = X0[i - 2] * X0[i - 1] - self.parameter[osc_ind, 2] * X0[i]
    ...
    def _cluster_oscillators(self, cluster_matrix):
        ...
        config = dict()
        config['sigma'] = self.parameter[::3]
        config['rho'] = self.parameter[1::3]
        config['beta'] = self.parameter[2::3]
    ...
    def _velocity_function(self, x0):
        ...
        for k in range(3, self.dimension + 3):
            ...
            if i % 3 == 0:
                ...
                dy_dx[k - 3] = X0[i - 1] + self.parameter[osc_ind, 0] * X0[i]
            elif i % 3 == 2:
                dy_dx[k - 3] = self.parameter[osc_ind, 1] + X0[i] * (X0[i - 2] - self.parameter[osc_ind, 2])
...
class Vanderpol2D(CoupledOscillator):
    ...
    def _velocity_function(self, x0):
        ...
        for k in range(2, self.dimension + 2):
            ...
            osc_ind = int(i / 2)
            if i % 2 == 0:
                dy_dx[k - 3] = self.parameter[osc_ind, 0] * (X0[i] - (X0[i] ^ 3) / 3 - X0[i + 1]) + self.epsilon * (X0[i + 3] - 2 * X0[i] + X0[i - 3])
            elif i % 2 == 1:
                dy_dx[k - 3] = 1./self.parameter[osc_ind, 0] * X0[i - 1]
